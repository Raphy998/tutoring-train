 ...
 */
public class AllOffersController implements Initializable, TutoringTrainWindow, MessageListener, RequestNewestOffersListener, RequestOfferCountListener, RequestOfferSearchListener {
    @FXML
    ...
    private void initializeControls() {
        ...
        comboStatus.getSelectionModel().select(EntryActiveState.ALL);
         ...
         try {
            if (validateInputControls()) {
                ...
                if (getDateOperation() != DateOperation.IGNORE) {
                    criteria.add(new DateSearchCriteria<>(getDateEntryProp(), getDateOperation(), getDate()));
                }
                if (getEntryState() != EntryActiveState.ALL) {
